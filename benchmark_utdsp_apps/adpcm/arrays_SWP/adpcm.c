...
void call_adpcm()
{
  ...
  for (c = 0; c < N; c++) {
...
#endif
      arrayIndex = 1-c;
    temp1 = quant_diff_sig[arrayIndex] * coeff_b[0];
    for (i = 1; i < 6 ; i++) {
      estim_zero_sig += temp1;
#ifdef GCCmod
      if ( i-c+1 < 0 )
...
#endif
        arrayIndex = i-c+1;
      temp1 = quant_diff_sig[arrayIndex] * coeff_b[i];
    }
    estim_zero_sig += temp1;
    ...
    if (tone_trans_detect_sig == 1) {
      ...
      temp7 = ((0.99609375) * coeff_a[0]) + ((1.171875e-2) * temp2);
...
#endif
          arrayIndex = 1-c;
      temp2 = quant_diff_sig[arrayIndex];
      temp3 = coeff_b[0];
      if (temp2 >= 0.0)
        temp4 = temp1;
      else
        temp4 = -temp1;
      for (i = 1; i < 6; i++) {
        coeff_b[i-1] = (temp5 * temp3) + (temp7 * temp4);
#ifdef GCCmod
        if ( i-c+1 < 0 )
          arrayIndex = (((i-c+1) % 8) + 8) % 8;
        else
      ...
      coeff_b[5] = (temp5 * temp3) + (temp7 * temp4);
    }
