...
int and2[64];
DecompressInfo and3[9];  
HuffmanTable b1[1],b4[1];
int z=0;

/*GetDht*/
    HuffmanTable **htblptr;
/*GetSof*/
    JpegComponentInfo *compptr3;
/*GetSos*/
    JpegComponentInfo *compptr4;
    ...
    DecompressInfo *dcPtr;
{
 ...
 /*	    fprintf (stderr, "Bogus DHT counts");  
   ...
   /* *htblptr = (HuffmanTable *)and1;  /* */
  if (*htblptr == (HuffmanTable *)null2 ) {
        if(z==0)
          {*htblptr = (HuffmanTable *) b1; z++; /*malloc (sizeof (HuffmanTable));*/}
          else 
          {*htblptr = (HuffmanTable *) b4;}
    /*     *htblptr = (HuffmanTable *)and1; */
...
/* 	    fprintf (stderr, "Bogus table number %d", n);  
	...
	if ((*(dcPtr->quantTblPtrs+n)) == null2 ) {
	    /*dcPtr->quantTblPtrs[n] = (int *) malloc (64*sizeof (int));*/
            *(dcPtr->quantTblPtrs+n) = (int *) and2; 
	}  
 ...
 * GetApp0 --
 *
 *	Process an APP0 marker (probably a JFIF header).
 *
 * Results:
 *	None
 *
...
#define JFIF_LEN 14
	...
	if (*(b+0)==74 && *(b+1)==70 && *(b+2)==73 && *(b+3)==70)
            /*(strncmp (b, "JFIF", 5) == 0)*/ {
	    /*
    ...
    int code;
{
 ...
 /*	fprintf (stderr, "Bogus SOF length");  */
    ...
    dcPtr->compInfo = (JpegComponentInfo *)and3; /* malloc
	(dcPtr->numComponents * sizeof (JpegComponentInfo));  */ 

    for (ci = 0; ci < dcPtr->numComponents; ci++) {
	compptr3 = dcPtr->compInfo+ci;     /* changed */
	compptr3->componentIndex = ci;
	compptr3->componentId = GetJpegChar();
	c = GetJpegChar();
	compptr3->hSampFactor = (c >> 4) & 15;
	compptr3->vSampFactor = (c) & 15;
	compptr3->quantTblNo = GetJpegChar();
    }
}

/*
    ...
    DecompressInfo *dcPtr;
{
  ...
  /*	fprintf (stderr, "Bogus SOS length");  */
   ...
   /*	    fprintf (stderr, "Invalid component number in SOS");  */
	...
	compptr4 = dcPtr->compInfo+ci;
	*(dcPtr->curCompInfo+i) = compptr4;
	compptr4->dcTblNo = (c >> 4) & 15;
	compptr4->acTblNo = (c) & 15;
    }
