...
LOOP_FP_CONV_O:
    ...
    LOOP_RESET_LINEBUFFERS:
        ...
        {
        PROLOG_COLS:
            for (IdxType c = 0; c < S; ++c)
            {
            PROLOG_ROWS:
                for (IdxType r = 0; r < K / 2; ++r)
                {
    ...
    // begin convolution
    LOOP_CONV_ROWS:
        for (IdxType r = 0; r < S + 1; ++r)
        {
        LOOP_CONV_COLS:
            for (IdxType c = 0; c < S + 1; ++c)
            {
                // load input word
                Word inword = 0;
                if (r < S && c < S)
...
LOOP_WT_I:
    for (Address i = 0; i < C_WT_WORDS * CONVOLVERS; ++i)
    {
...
LOOP_KH_I:
    for (ap_uint<16> i = 0; i < KH_WORDS; ++i)
        kh_mem[i] = kh_i[i];
