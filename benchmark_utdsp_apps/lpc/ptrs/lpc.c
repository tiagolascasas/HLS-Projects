...
main()
{
  ...
  output_dsp(window, N, 0);	/* Buffer for storing speech samples */
  output_dsp(hamming, N, 0);	/* Hamming Window */
  output_dsp(filtered, N, 0); 	/* low pass filtered speech */
  output_dsp(signal, N, 0);	/* Stores windowed speech samples */
  output_dsp(decimated, D, 0);	/* decimated filtered speech */
  output_dsp(IFout, D, 0);	/* Inverse filter output */
  output_dsp(autoc, D, 0);	/* Autocorrelation of IFout */
  output_dsp(alpha, P, 0);	/* Predictor coefficients */
  output_dsp(autocorr, P, 0);	/* Stores autocorrelation vector */
  output_dsp(temp_vect, P, 0);	/* Temporary vector */
  output_dsp(coeff, IF, 0);	/* Inverse filter coefficients */
  output_dsp(temp_filt, N, 0);	/* Temporary filtered signals. */

  output_dsp(&gain, 1, 0);	/* Gain of the speech signal */
  output_dsp(&pitch, 1, 0);	/* Pitch of the speech signal */
  output_dsp(&voiced, 1, 0);	/* Flag for voiced or unvoiced speech */
}
...
void SIFT()
{
  ...
  /* Older Version: Separate pointers into the temp_filt array (with *b and *c)
     obscure the dependence between *b and *c causing the operation compaction
     phase to incorrectly schedule the load of "temp2=*c++" before the store
     of "*b++=temp5".  A similar problem occurs because the separate pointers
     into the filter array (with *d and *e) obscure the dependence between
     "*d++=..." and "temp3=*e++".
  */
  ...
  output_dsp(window, D, 0);
  output_dsp(temp_filt, D, 0);
  output_dsp(filtered, D, 0);
  ...
  *f++ = f_1 = (a3 * t0) + (a4 * f_1);
  ...
  t0 = (a1 * temp1) + (a2 * t_1);
  *t++ = t0;
  *f++ = (a3 * t0) + (a4 * f_1) + (a5 * f_2);

  output_dsp(filtered, N, 0);
#endif
