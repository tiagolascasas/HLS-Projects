...
int DSP_dotprod_graph(const short *x, const short *y, int nx)
{
	...
	int sum = 0, i;
     ...
     n_const++;
	 fprintf(f,"const%d [label=\"0\"];\n",n_const);
	 fprintf(f,"\"sum_%d\" [label=\"sum\"];\n",n_sum+1);
	 ne++;
    ...
    for (i = 0; i < nx; i++){
        sum += x[i] * y[i];
	  	...
	  	fprintf(f,"\"x[%d]_%d_l\" [label=\"x[%d]\"];\n" ,i, n_x[i] ,i);//nc-5
		fprintf(f,"\"y[%d]_%d_r\" [label=\"y[%d]\"];\n" ,i, n_y[i] ,i);//nc-5
		n_op++;
		fprintf(f,"op%d [label=\"*\", att1=op];\n",n_op);//nc-3
		n_temp++;
		fprintf(f,"temp%d [label=\"temp_l83_i%d\"];\n",n_temp,n_temp);//nc-2
		   
		ne++;
		fprintf(f,"\"x[%d]_%d_l\"->op%d [label=\"%d\", ord=\"%d\"];\n",i, n_x[i],n_op,ne,ne);
		ne++;
		fprintf(f,"\"y[%d]_%d_r\"->op%d [label=\"%d\", ord=\"%d\"];\n",i,n_y[i],n_op,ne,ne);
		ne++;
