...
/*static*/ /*long*/int getBuffer;          /* current bit-extraction buffer */
 ...
 */
/*uchar*/int inputBuffer[JPEG_BUF_SIZE]; /* Input buffer for JPEG data */
...
/*HuffDecoderInit*/
    JpegComponentInfo *compptr1;
/*DecodeMCUrle*/
    RLE *rle2;
    HuffmanTable *dctbl;
    HuffmanTable *actbl;
    JpegComponentInfo *compptr2;
    ...
    HuffmanTable *htbl;
{
    ...
    for (l = 1; l <= 16; l++) {
        ban1=htbl->bits;
        for (i = 1; i <= *(ban1+l)/*(int)htbl->bits[l]*/; i++)
            {
            ban22=huffsize;
            /*huffsize[p++]*/ *(ban22+p) = /*(char)*/l;
            p++;
            }
    }
    ban3=huffsize;
    ...
    for(b=0;b<256;b++) htbl->numbits[b]=0;
...
/*
 * bmask2[n] is mask for n rightmost bits
 */
/*static*/ int bmask2[] = {0x0000,
...
#define show_bits(nbits,rv) {                                           \
    ...
    rv = (getBuffer >> (bitsLeft-(nbits))) & *(bmask2+nbits);              \
}
    ...
    DecompressInfo *dcPtr;
{
    ...
    for (ci = 0; ci < dcPtr->compsInScan; ci++) {
        compptr1 = *(dcPtr->curCompInfo+ci);
        /*
         * Make sure requested tables are present
         */
        /*if ((dcPtr->dcHuffTblPtrs[compptr1->dcTblNo] == NULL) ||
            (dcPtr->acHuffTblPtrs[compptr1->acTblNo] == NULL)) {
         ...
         * big deal
         */
        FixHuffTbl (*(dcPtr->dcHuffTblPtrs+(compptr1->dcTblNo)));
        FixHuffTbl (*(dcPtr->acHuffTblPtrs+(compptr1->acTblNo)));
    ...
    MCU mcuRLE;
{
    ...
    for (blkn = 0; blkn < dcPtr->blocksInMCU; blkn++) {
        ...
        compptr2 = *(dcPtr->curCompInfo+ci);
        actbl = *(dcPtr->acHuffTblPtrs+(compptr2->acTblNo));
        dctbl = *(dcPtr->dcHuffTblPtrs+(compptr2->dcTblNo));
