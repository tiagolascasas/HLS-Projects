                              ...
                              unsigned n_inputs, unsigned n_outputs)
{
    ...
    if (imgs_per_batch > n_outputs)
        imgs_per_batch = n_outputs;
    if (imgs_per_batch > wt_bsize)
        imgs_per_batch = wt_bsize;
    if (imgs_per_batch > kh_bsize)
        imgs_per_batch = kh_bsize;
...
unsigned find_dense_batch_size(unsigned n_inputs, unsigned n_outputs)
{
    ...
    // adjust output batch size to fit into memories cleanly
    if (bits_per_batch > n_outputs)
        bits_per_batch = n_outputs;
    if (bits_per_batch > wt_bsize)
        bits_per_batch = wt_bsize;
    if (bits_per_batch > kh_bsize)
        bits_per_batch = kh_bsize;
